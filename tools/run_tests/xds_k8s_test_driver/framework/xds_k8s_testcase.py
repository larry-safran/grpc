# Copyright 2020,2021,2022 gRPC authors.
#
...
class IsolatedWithoutCPXdsKubernetesTestCase(XdsKubernetesBaseTestCase,
  ...
  """Should be called by the implementations of cleanup."""
  def cleanupClientAndServer(self):
    self.client_runner.cleanup(force=self.force_cleanup)
    self.server_runner.cleanup(force=self.force_cleanup,
                               force_namespace=self.force_cleanup)


class IsolatedXdsKubernetesTestCase(IsolatedWithoutCPXdsKubernetesTestCase,
                                    metaclass=abc.ABCMeta):
  """Isolated test case.
...
class FakeControlPlaneXdsKubernetesTestCase(IsolatedWithoutCPXdsKubernetesTestCase,
  ...
  Base class for tests cases where infra resources are created before
  each test, and destroyed after that connects to a fake control plane
  instead of traffic director.
  """
